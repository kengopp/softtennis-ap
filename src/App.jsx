...
let _matches = [];
let _nid = 1;
const uid    = () => `id_${_nid++}_${Math.random().toString(36).slice(2)}`;
const today  = () => new Date().toISOString().slice(0, 10);
const fmtDate = (iso) => iso ? iso.replace(/-/g, "/") : "";

function seedDemo() {
  const mid = uid(), g1 = uid(), g2 = uid(), g3 = uid();
  _matches = [{
    id: mid, created_by: "me",
    match_date: "2026-06-15", venue: "○○市民コート",
    tournament_name: "○○中学校選手権", round: "準々決勝",
    match_type: "tournament", game_format: 7,
    is_doubles: true, first_server: "A", status: "finished",
    match_score_a: 2, match_score_b: 1, memo: "",
    players: [
      { id: uid(), team: "A", player_name: "田中 蓮",   club_name: "○○中学校",  position: "back", order_num: 1 },
      { id: uid(), team: "A", player_name: "佐藤 健",   club_name: "○○中学校",  position: "fore", order_num: 2 },
      { id: uid(), team: "B", player_name: "鈴木 一郎", club_name: "新宮中学校", position: "back", order_num: 1 },
      { id: uid(), team: "B", player_name: "山田 健二", club_name: "新宮中学校", position: "fore", order_num: 2 },
    ],
    games: [
      {
        id: g1, match_id: mid, game_number: 1, server_team: "A",
        is_final: false, score_a: 4, score_b: 2, winner_team: "A",
        faults: [],
        points: [
          { id: uid(), game_id: g1, match_id: mid, point_number: 1, scoring_team: "A", player_name: "田中 蓮",   play_type: "serve",   result_type: "ace",    is_winner: true,  score_a_after: 1, score_b_after: 0 },
          { id: uid(), game_id: g1, match_id: mid, point_number: 2, scoring_team: "A", player_name: "佐藤 健",   play_type: "volley",  result_type: "winner", is_winner: true,  score_a_after: 2, score_b_after: 0 },
          { id: uid(), game_id: g1, match_id: mid, point_number: 3, scoring_team: "B", player_name: "鈴木 一郎", play_type: "stroke",  result_type: "winner", is_winner: true,  score_a_after: 2, score_b_after: 1 },
          { id: uid(), game_id: g1, match_id: mid, point_number: 4, scoring_team: "A", player_name: "田中 蓮",   play_type: "stroke",  result_type: "winner", is_winner: true,  score_a_after: 3, score_b_after: 1 },
          { id: uid(), game_id: g1, match_id: mid, point_number: 5, scoring_team: "B", player_name: "山田 健二", play_type: "volley",  result_type: "error",  is_winner: false, score_a_after: 4, score_b_after: 1 },
          { id: uid(), game_id: g1, match_id: mid, point_number: 6, scoring_team: "A", player_name: "佐藤 健",   play_type: "smash",   result_type: "winner", is_winner: true,  score_a_after: 4, score_b_after: 2 },
        ],
      },
      {
        id: g2, match_id: mid, game_number: 2, server_team: "B",
        is_final: false, score_a: 2, score_b: 4, winner_team: "B",
        faults: [
          { id: uid(), game_id: g2, match_id: mid, fault_number: 1, server_team: "A", player_name: "田中 蓮", score_a_at: 1, score_b_at: 3 },
        ],
        points: [
          { id: uid(), game_id: g2, match_id: mid, point_number: 1, scoring_team: "B", player_name: "山田 健二", play_type: "volley",  result_type: "winner", is_winner: true,  score_a_after: 0, score_b_after: 1 },
          { id: uid(), game_id: g2, match_id: mid, point_number: 2, scoring_team: "A", player_name: "田中 蓮",   play_type: "lob",     result_type: "winner", is_winner: true,  score_a_after: 1, score_b_after: 1 },
          { id: uid(), game_id: g2, match_id: mid, point_number: 3, scoring_team: "B", player_name: "鈴木 一郎", play_type: "stroke",  result_type: "winner", is_winner: true,  score_a_after: 1, score_b_after: 2 },
          { id: uid(), game_id: g2, match_id: mid, point_number: 4, scoring_team: "B", player_name: "山田 健二", play_type: "smash",   result_type: "winner", is_winner: true,  score_a_after: 1, score_b_after: 3 },
          { id: uid(), game_id: g2, match_id: mid, point_number: 5, scoring_team: "A", player_name: "田中 蓮",   play_type: "stroke",  result_type: "error",  is_winner: false, score_a_after: 2, score_b_after: 3 },
          { id: uid(), game_id: g2, match_id: mid, point_number: 6, scoring_team: "B", player_name: "鈴木 一郎", play_type: "serve",   result_type: "error",  is_winner: false, score_a_after: 2, score_b_after: 4 },
        ],
      },
      {
        id: g3, match_id: mid, game_number: 3, server_team: "A",
        is_final: false, score_a: 4, score_b: 3, winner_team: "A",
        faults: [],
        points: [
          { id: uid(), game_id: g3, match_id: mid, point_number: 1, scoring_team: "A", player_name: "田中 蓮",   play_type: "stroke",  result_type: "winner", is_winner: true,  score_a_after: 1, score_b_after: 0 },
          { id: uid(), game_id: g3, match_id: mid, point_number: 2, scoring_team: "B", player_name: "鈴木 一郎", play_type: "lob",     result_type: "winner", is_winner: true,  score_a_after: 1, score_b_after: 1 },
          { id: uid(), game_id: g3, match_id: mid, point_number: 3, scoring_team: "A", player_name: "佐藤 健",   play_type: "volley",  result_type: "winner", is_winner: true,  score_a_after: 2, score_b_after: 1 },
          { id: uid(), game_id: g3, match_id: mid, point_number: 4, scoring_team: "B", player_name: "山田 健二", play_type: "volley",  result_type: "error",  is_winner: false, score_a_after: 3, score_b_after: 1 },
          { id: uid(), game_id: g3, match_id: mid, point_number: 5, scoring_team: "B", player_name: "鈴木 一郎", play_type: "stroke",  result_type: "winner", is_winner: true,  score_a_after: 3, score_b_after: 2 },
          { id: uid(), game_id: g3, match_id: mid, point_number: 6, scoring_team: "B", player_name: "山田 健二", play_type: "drop",    result_type: "winner", is_winner: true,  score_a_after: 3, score_b_after: 3 },
          { id: uid(), game_id: g3, match_id: mid, point_number: 7, scoring_team: "A", player_name: "佐藤 健",   play_type: "smash",   result_type: "winner", is_winner: true,  score_a_after: 4, score_b_after: 3 },
        ],
      },
    ],
  }];
...
function saveMatch(m) {
  const i = _matches.findIndex(x => x.id === m.id);
  if (i >= 0) _matches[i] = m; else _matches.unshift(m);
}
function deleteMatch(id) {
  _matches = _matches.filter(m => m.id !== id);
}
function getMatches() { return [..._matches]; }
function getMatch(id) { return _matches.find(m => m.id === id) ?? null; }
...
// 試合一覧
// ============================================================
function MatchList({ onNew, onOpen, onCopy }) {
  const [filter, setFilter] = useState("all");
  const [tick,   setTick]   = useState(0);
  const [confirmDelete, setConfirmDelete] = useState(null); // 削除確認対象のmatch_id
  const matches = getMatches().filter(m=>filter==="all"||m.match_type===filter);
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ padding:"12px 14px" }} key={tick}>
        {matches.length===0 && <div style={{ textAlign:"center",color:C.textSec,marginTop:60 }}><div style={{ fontSize:40,marginBottom:12 }}>🎾</div>試合記録がありません</div>}
        {matches.map(m=>{
...
function MatchSetup({ onSave, onCancel, sourceMatchId, editMatchId }) {
  // モード判定： editMatchId（既存試合の編集）/ sourceMatchId（コピー元あり）/ 通常新規
  const editing = editMatchId ? getMatch(editMatchId) : null;
  const source  = sourceMatchId ? getMatch(sourceMatchId) : null;
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ padding:14 }}>
        ...
        <button
          style={{ ...S.btn(canSave ? `linear-gradient(135deg,${C.accent},#00a066)` : C.border, canSave ? C.white : C.textSec), marginTop:4 }}
          disabled={!canSave}
          onClick={handleSave}
        >
          {editing ? "保存する 💾" : "試合を開始する 🎾"}
        </button>
      </div>
    </div>
  );
...
function ScoreRecord({ matchId, onBack, onEdit }) {
  ...
  const persist = useCallback((m)=>{saveMatch(m);setMatch({...m});},[]);
